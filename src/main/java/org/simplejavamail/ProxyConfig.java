...
public class ProxyConfig {
	...
	/**
	 * Forces Simple Java Mail to skip proxy, even if it is configured in config file.
	 */
	public static final ProxyConfig SKIP_PROXY_CONFIG = new ProxyConfig();
	...
	public ProxyConfig(String remoteProxyHost, Integer remoteProxyPort, String username, String password) {
		this.remoteProxyHost = valueOrProperty(remoteProxyHost, PROXY_HOST);
		this.remoteProxyPort = valueOrProperty(remoteProxyPort, PROXY_PORT);
		this.username = valueOrProperty(username, PROXY_USERNAME);
		this.password = valueOrProperty(password, PROXY_PASSWORD);
		this.proxyBridgePort = valueOrProperty(null, PROXY_SOCKS5BRIDGE_PORT, DEFAULT_PROXY_BRIDGE_PORT);
