...
public class MailTestDemoApp {
	...
	public static void main(final String[] args)
		...
		final EmailBuilder emailBuilderNormal = EmailBuilder.builder();
		emailBuilderNormal.from("lollypop", "lol.pop@somemail.com");
		// don't forget to add your own address here ->
		emailBuilderNormal.to("C.Cane", YOUR_GMAIL_ADDRESS);
		emailBuilderNormal.text("We should meet up!");
		emailBuilderNormal.textHTML("<b>We should meet up!</b><img src='cid:thumbsup'>");
		emailBuilderNormal.subject("hey");

		// add two text files in different ways and a black thumbs up embedded image ->
		emailBuilderNormal.addAttachment("dresscode.txt", new ByteArrayDataSource("Black Tie Optional", "text/plain"));
		emailBuilderNormal.addAttachment("location.txt", "On the moon!".getBytes(Charset.defaultCharset()), "text/plain");
		...
		emailBuilderNormal.embedImage("thumbsup", parseBase64Binary(base64String), "image/png");
		...
		sendMail(emailFromMimeMessage); // should produce the exact same result as emailBuilderNormal!
	}
