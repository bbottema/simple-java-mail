...
All published library modules remain Java 8 source, API, bytecode, and runtime compatible. The separately executable `cli-module` requires Java 17 or newer. A full-reactor build therefore runs on JDK 17+ and compiles non-CLI modules with `--release 8` while compiling only `cli-module` with `--release 17`.

Use a real JDK 8 lane when validating library compatibility. That lane excludes `cli-module`; JDK 8 cannot load its Java 17 classes. CLI metadata generation now runs on JDK 17 or the current release JDK. The Kryo cache has explicit serializers for reflective methods and Therapi's read-only collection shapes, and does not reflect into JDK internals.
...
Library-only compatibility validation on JDK 8:
