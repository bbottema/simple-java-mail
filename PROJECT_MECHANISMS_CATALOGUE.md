...
| Configuration provenance diagnostics | Retain each winning property's source while resolving the immutable snapshot, then expose grouped log-safe values without confusing factory input with a built Mailer's effective configuration. | [ConfigDiagnostics.java](modules/core-module/src/main/java/org/simplejavamail/config/ConfigDiagnostics.java), [PropertySchema.java](modules/core-module/src/main/java/org/simplejavamail/config/PropertySchema.java), [SpringEnvironmentConfigSource.java](modules/spring-module/src/main/java/org/simplejavamail/springsupport/SpringEnvironmentConfigSource.java) |
...
- `SimpleJavaMail.getConfig()` returns the exact snapshot held by the factory. Its raw getters contain real configured values; `getDiagnostics()` is the grouped, redacted view intended for logs.
- `ConfigSource` names appear in provenance and validation errors. A composite source can name the underlying source separately for each property. Error messages name the key and expected type without echoing the configured value.
- `ConfigDiagnostics` is captured while winning scalar and wildcard values are resolved, so extra Jakarta Mail settings and connection-pool cluster fields retain individual origins. It does not include later builder overrides or generated runtime defaults; inspect `Mailer.getOperationalConfig()` for the effective operational state of a built Mailer.
...
`SmtpConnectionProbe` selects an optional `SmtpConnectionProbeAdapter` through ServiceLoader against the exact selected provider. Unsupported providers and CustomMailer return an explicit unsupported report without connecting. The probe clones Session properties and retains the selected provider registration without editing the original. In no-auth mode the private Session has no authenticator or cached credentials, and never invokes the OAuth2 token provider. In auth mode a lazy bridge retains the caller's authenticator/cache and the ordinary `TransportConnectionHelper` retains OAuth2 resolution. Authentication settings and TLS policy for ordinary sends are unchanged.

The Angus adapter owns one inspection-only `AngusProbeTransport`. Protected greeting/EHLO/STARTTLS/AUTH hooks capture separate immutable facts; no raw transcript or provider exception is kept. A rejected post-TLS EHLO ends the probe before Angus can reuse pre-TLS advertisements to authenticate. HELO-only discovery and overlarge EHLO replies leave capabilities unknown. `SmtpCapabilities` preserves repeated and unknown extensions, while its SIZE helper only returns consistent positive numeric limits. These are advertisements, never promises that a message used the extensions.

`AngusProbeTlsObserver` uses the supported hostname-verifier hook on the private Session. It preserves an application's object verifier and its veto; the provider's native endpoint identification and later trust checks still apply. A verifier configured by class name is left untouched, with unavailable metadata instead of duplicated private class-loading logic. TLS protocol/cipher/public identities and configured identity checking are not an independent verified-trust verdict. `SmtpConnectionReport` contains bounded escaped display text, safe phase-specific failures, and no Email, credentials, authentication replies, raw exceptions or transport references.

Both paths reuse `AbstractProxyServerSyncingClosure` for proxy accounting. The synchronous probe holds the existing Mailer monitor through cleanup so shutdown cannot close its shared proxy mid-probe; an accepted async wrapper calls that same path on the Mailer's executor. Admission/shutdown failures are exceptional futures, while connection/cleanup failures are reports. Provider I/O timeouts apply; the total mail-send deadline does not. Probes are serialized per Mailer, and future cancellation does not abort network I/O. The ordinary send pool and send state machines are unchanged.

No new persistent setting or Email governance field is introduced: endpoint, proxy, TLS and timeout configuration already exists, and the authentication choice belongs to one diagnostic invocation. Spring's injected Mailer can use the same API. The local [demo](modules/simple-java-mail/src/test/java/demo/SmtpConnectionProbeDemoApp.java) and [integration tests](modules/simple-java-mail/src/test/java/org/simplejavamail/mailer/internal/SmtpConnectionProbeTest.java) exercise this path separately from sending. Logging-only mode retains supplied regular-builder SMTP settings for explicit probes; sending itself still requires no configured host and does not connect.

The CLI's `probe` command calls the synchronous view and writes the same report to the request-owned output stream. Its `--authenticate` flag is a command choice stored separately from generated builder options, so it never changes the Mailer profile or a later request's authentication choice. Both one-shot and daemon routes return the report on stdout, with exit code 0 for success or 3 for a failed/unsupported report; argument errors remain 2. The existing Mailer lease surrounds probing and reporting, while each probe's dedicated transport is closed before its report is returned. No daemon protocol, send-pool or observer changes are needed.

The separately packaged [third-party probe fixture](modules/simple-java-mail/src/test/third-party-probe/org/simplejavamail/thirdpartyprobe/ThirdPartyProbeConsumer.java) verifies runtime discovery through both `META-INF/services` and JPMS `provides`, with Angus absent. Its synthetic Jakarta Mail provider exercises authentication, Session isolation and cleanup without network I/O. A successful connection can have unavailable EHLO or TLS details; the adapter must omit those optional facts rather than invent an empty capability snapshot. Adapters also own secret filtering: escaping report text is not a substitute for keeping raw provider exceptions and AUTH exchanges out of it. The fixture uses only public APIs and is never included in the published artifacts.

## Async Send And Batch Connection Pooling

`Mailer.sync()` and `Mailer.async()` return cached, immutable adapters over one `MailerImpl`. They allocate no execution resources, own no lifecycle and use the owning Mailer's synchronization. Each exposes send, simple batch, connection test and probe operations. Single-email sync sends return the exact `MailSubmissionReceipt`; async sends return `MailSend<MailSubmissionReceipt>` directly, without a receipt-discarding completion wrapper. Simple batches are lazy and first-failure-stopping: sync returns void, async returns `MailSend<Void>`, and no receipt history is accumulated. The builder async flag and flat/default/boolean-mode methods are removed. `getCompletion()` creates detached CompletableFuture views of the actual operation; mutations of a view cannot affect sending. `requestCancellation()` is an optional idempotent void request, not an acknowledgement. `MailSendOperation` still arbitrates execution versus queued retirement on either executor. Connection tests retain their dedicated helper path. Async closures acquire resources only after worker execution begins. CLI send/connect use the synchronous view; daemon request concurrency stays outside the Mailer. Session logging is mode-neutral.

Key pieces:

- `MailerGenericBuilderImpl` creates a Mailer-local `MailSendExecutor`. With `batch-module` present it uses the configured worker size and keep-alive; without it, the existing single-worker, no-idle-expiry behavior remains. Workers are non-daemon even when the submitting thread is a daemon.
- `AsyncQueueConfig` separates waiting capacity from worker count. Capacity -1 remains unbounded, zero uses a direct handoff, and positive values use a bounded FIFO queue. `REJECT` fails immediately; `WAIT_FOR_CAPACITY` waits on the caller for a bounded interval without running SMTP there. Shutdown and interruption also stop admission waits.
- Java builder settings and `simplejavamail.defaults.async.queue.*` properties share the typed configuration schema and Spring metadata. A simple batch consumes one executor slot; its iterable remains lazy. The bound does not count concurrent caller-side preparation or limit the number of emails inside a batch.
- Queue defaults belong to `MailerGenericBuilder`. Its implementation resolves the property-backed settings and builder overrides, then passes the immutable `AsyncQueueConfig` into `OperationalConfigImpl`. The configuration getter returns that supplied instance; neither configuration type manufactures defaults.
- `MailSendRejectedException` retains a typed rejection reason. Ordinary rejected attempts notify the observer exactly once on the caller thread, with no receipt or started timestamp. Rejected batches do not open their iterable or notify untouched emails.
- `Mailer.getAsyncQueueSnapshot()` returns detached, content-free queue/worker estimates and cumulative rejection counts. `MailSendOutcome` continues to provide per-attempt ready/started timestamps for queue delay.
- Caller-provided executors retain their own policy and lifecycle; non-default built-in queue settings are rejected when combined with one. `resetAsyncQueue()` explicitly discards those settings. Standalone `BatchTransportExecutor` remains separate and still uses `NonJvmBlockingThreadPoolExecutor` by default; the Mailer queue setting does not configure its queue.
- The internal `BatchModule` bridge only registers clusters, acquires pooled transports and closes connection pools; it does not schedule async operations or create executors. `BatchSupport.registerToCluster(...)` creates/registers SMTP connection pools using `SmtpConnectionPoolClustered`.
...
Adapters catch checked `MessagingException` failures and return `MailTransportResult.failed(...)`. That factory extracts the valid-sent, valid-unsent, and invalid arrays from a `SendFailedException` when present, clones every array on input and output, and retains the original exception. When a failed transport call cannot determine final server acceptance but still has recipient facts that remain certain, `failedWithUnknownAcceptance(...)` forces `UNKNOWN` without discarding those facts. Adapters can enrich the result with `withRecipientResults(...)`; `withEnvelopeRecipients(...)` fills in ordered snapshots for older adapters without inventing RCPT details. `TransportRunner` builds either a `MailSubmissionReceipt` or a `MailSubmissionException` whose direct cause is that original Jakarta Mail failure. Rich receipts derive the old accepted/unsent/invalid lists from the same immutable recipient results; legacy constructors and serialized receipts remain readable.

The Angus adapter temporarily enables success reporting on the exclusively used `SMTPTransport`, inside the transport monitor, and restores its previous setting in `finally`. It never mutates the shared Session's properties. `AngusSubmissionResult` reads the typed `SMTPAddressSucceededException`, `SMTPAddressFailedException`, and `SMTPSendFailedException` chain, matches replies to envelope occurrences, and snapshots reply text. A direct reporting exception becomes normal success only with final 2xx acceptance, all recipients accepted, and no rejection. Genuine failures retain the original exception. Enhanced statuses are parsed from reply text, but acceptance and retry classification use structured primary codes and recipient groups, never prose matching.

`MailRecipientResult` separates the RCPT-level `SmtpRecipientStatus` from final `MailRecipientDisposition`. `MailRetryDisposition` is guidance, not a retry engine: known unsent recipients exclude permanent RCPT rejections, permanent transaction rejection prevents resend advice, and missing final acceptance or accepted/retryable occurrences of the same mailbox indicate duplicate risk. Unknown custom-mailer and logging-only results require caller policy. `getRetryableRecipients()` exposes candidates only for the two safe-to-retry dispositions. The fields are result-only, so no builder, configuration, Spring property, or CLI option is added.
...
- A positive RCPT reply confirms only that recipient command, not the subsequent DATA transaction. Missing replies stay absent, including provider EOF/zero sentinels. A provider can lose its internal RCPT chain during some I/O failures; never reconstruct those missing replies from the last connection response.
- Recipient addresses and raw replies are potentially sensitive, untrusted data, unlike the redacted configuration diagnostic report. Applications own logging and escaping them.
