...
/**
 * Mailing tool aimed for simplicity, for sending emails of any complexity. This includes emails with plain text and/or
 * html content, embedded images and separate attachments<br />
 * <br />
 * This mailing tool abstracts the javax.mail API to a higher level easy to use API. For public use, this tool only
 * works with {@link Email} instances. <br />
 * <br />
 * The email message structure is built to work with all email clients and has beed tested with many different
 * webclients as well as some mainstream client applications such as MS Outlook or Mozilla Thunderbird.<br />
 * <br />
...
public class Mailer {
	 ...
	 * this mailer directly, when no <code>Session</code> was provided.
	 * 
	 * @see #Mailer(Session)
	...
	/**
	 * Overloaded constructor which produces a new {@link Session} on the fly. Use this if you don't have a mail session
	...
	/**
	 * Processed an {@link Email} instance into a completely configured {@link Message}, which in turn is being sent to
	 * all defined recipients.
	 * 
	 * @param email The information for the email to be sent.
	 * @throws MailException Can be thrown if an email isnt validating correctly, or some other problem occurs during
	...
	/**
	 * Validates an {@link Email} instance. Validation fails if the subject is missing, content is missing, or no
	...
	public boolean validate(final Email email)
		...
		if (email.getText() == null && email.getTextHTML() == null) {
			...
			if (!EmailValidationUtil.isValid(email.getFromRecipient().getAddress())) {
				throw new MailException(String.format(MailException.INVALID_SENDER, email));
			}
			for (final Recipient recipient : email.getRecipients()) {
				if (!EmailValidationUtil.isValid(recipient.getAddress())) {
	...
	/**
	 * Creates a new {@link MimeMessage} instance and prepares it in the email structure, so that it can be filled and
	 * send.
	 * 
	...
	/**
	 * Helper method which generates a {@link BodyPart} from an {@link AttachmentResource} (from its {@link DataSource})
	 * and a disposition type ({@link Part#INLINE} or {@link Part#ATTACHMENT}). With this the attachment data can be
	 * converted into objects that fit in the email structure. <br />
	 * <br />
